...
class Post {
	...
	public function submitPost($body, $user_to_ID, $imageName) {
		...
		if($check_empty != "" || $imageName != "") {
			...
			//Get username
			$added_by_ID = $this->user_obj->getMemberID();

			//If user is on own profile, user_to_ID is '0'
			if($user_to_ID == $added_by_ID) {
				$user_to_ID = "0"; // 0 (zero) = "none"
			}
			...
			$num_posts++;
			$update_query = mysqli_query($this->con, "UPDATE users SET num_posts='$num_posts' WHERE memberID='$added_by_ID'");
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($user_logged_obj->isFriend($added_by_ID)) {
					...
					if($user_row['memberType'] == "Fan" || $user_row['memberType'] == "Musician") {
	...
	public function getSinglePost($post_id) {
		...
		$opened_query = mysqli_query($this->con, "UPDATE notifications SET opened='yes' WHERE user_to_ID='$userLoggedInID' AND link LIKE '%=$post_id'");
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				if($user_logged_obj->isFriend($added_by_ID)){
					...
					$user_details_query = mysqli_query($this->con, "SELECT first_name, last_name, profile_pic FROM users WHERE memberID='$added_by_ID'");
					$user_row = mysqli_fetch_array($user_details_query);
					$first_name = $user_row['first_name'];
					$last_name = $user_row['last_name'];
					$profile_pic = $user_row['profile_pic'];
